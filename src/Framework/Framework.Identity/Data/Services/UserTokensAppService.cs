...
namespace Framework.Identity.Data.Services
{
    public class UserTokensAppService
    {
        ...
        {
            var UserToken = await _userTokensRepository.TableNoTracking.Where(s => s.UserId == UserId).FirstOrDefaultAsync();
            if (UserToken != null)
            {
                await _userTokensRepository.DeleteAsync(UserToken.UserId, true);
                return true;
            }
            return false;
        }

        public async Task<bool> DeleteByJTIAsync(string jti)
        {
            var UserToken = await _userTokensRepository.TableNoTracking.Where(s => s.LoginProvider == jti).FirstOrDefaultAsync();
            if (UserToken != null)
            {
                await _userTokensRepository.DeleteAsync(UserToken.UserId, true);
                return true;
