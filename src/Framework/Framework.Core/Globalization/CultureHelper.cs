...
namespace Framework.Core.Globalization
{
    ...
    public static class CultureHelper
    {
        ...
        public static void InitializeCultureFromHeadersAr(HttpContext context)
        {
            var defaultCulture = "ar-SA";

            // The header format is "accept-language: ar-SA", where accept-language is header key
            var userLangs = context.Request.Headers["Accept-Language"].ToString();
            var language = userLangs.Split(',').FirstOrDefault();

            var cultureToSet = language.IsNotNullOrEmpty()? language : defaultCulture;
            var culture = GetCultureInfo(cultureToSet);
