...
namespace Framework.Core.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        public static string HidePartOfText(this string text, char replacedChar = '#', int skipFirstCount = 2, int skipLastCount = 2, bool isFullHide = false)
        {
            StringBuilder sb = new StringBuilder(text.Length);
            for (int i = 0; i < text.Length; i++)
            {
                if (!isFullHide && (i + 1) <= skipFirstCount)
                {
                    sb.Append(text[i]);
                }
                else if (!isFullHide && (i + 1) > (text.Length - skipLastCount))
                {
                    sb.Append(text[i]);
                }
                else sb.Append(replacedChar);
            }
            return sb.ToString();
        }
